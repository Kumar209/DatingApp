...
public class AdminController(UserManager<AppUser> userManager, IUnitOfWork uow,
    IPhotoService photoService, IAdminRepository adminRepository) : BaseApiController
{
    ...
    [Authorize(Policy = "RequireAdminRole")]
    [HttpGet("users-with-roles")]
    public async Task<ActionResult<PaginatedResult<ManagedUserDto>>> GetUsersWithRoles([FromQuery] ManagedUserParams managedUserParams)
    {
        var result = await adminRepository.GetUsersAsync(managedUserParams);
        return Ok(result);
    }
    ...
    public async Task<ActionResult> RejectPhoto(int photoId)
    {
        ...
        {
            //Delete photo from cloudinary first, then delete from db if successful
